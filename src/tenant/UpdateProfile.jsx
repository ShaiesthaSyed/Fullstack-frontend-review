...
export default function UpdateProfile() 
{
  ...
  useEffect(() => {
    ...
    if (storedTenant) {
      setFormData(JSON.parse(storedTenant));
    }
  }, []);
  ...
  const handleSubmit = async (e) => 
  {
    ...
    try 
    {
      const response = await axios.put(`${config.url}/tenant/updateprofile`, formData);
      if (response.status === 200) 
        {
        setMessage(response.data);
        setError('');
        sessionStorage.setItem('tenant', JSON.stringify(formData)); // Update local session data
      }
    } 
    catch (error) 
    {
